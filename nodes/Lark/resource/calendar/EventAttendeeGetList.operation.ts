...
export default {
	...
	options: [
		...
		{
			displayName: `<a target="_blank" href="https://open.larksuite.com/document/server-docs/calendar-v4/calendar-event-attendee/list">${WORDING.OpenDocument}</a>`,
			name: 'notice',
			type: 'notice',
			default: '',
		},
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject[]> {
		...
		const needResourceCustomization = options.need_resource_customization as boolean | undefined;
		...
		do {
			const {
				...
				qs: {
					...
					...(needResourceCustomization !== undefined && {
		...
		const list = (allAttendees ?? []) as IDataObject[];
