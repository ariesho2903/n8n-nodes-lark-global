...
export default {
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		const rawUserId = this.getNodeParameter('user_id', index, undefined, {
			extractValue: true,
		});
		const user_id = typeof rawUserId === 'string' ? rawUserId : String(rawUserId);

		const selectedUserIdType = this.getNodeParameter('user_id_type', index, 'open_id') as string;
		...
		const {
			...
			qs: {
				user_id_type: selectedUserIdType,
